...
def generate_html_report(repo_reports, summary_report, filename):
    ...
    <body>
        <a href="{filename.replace('.html', '.xlsx')}" download>Download as Excel</a>
        <div class="tab">
            """ + tabs + """
        </div>
        """ + contents + """
...
def generate_report_for_repo(repo, include_estimates):
    try:
        ...
        html_content = generate_html_content(
            ...
            scaling_policies, compliance_requirements, documentation, manifest_details, pcf_references, java_versions, angular_versions, node_versions, pcf_recommendations, complexity, dev_days, qa_days, include_estimates
        )
    ...
    summary_content = f"""
        ...
        <table>
            <tr>
                ...
                <th>Node Versions</th>
                <th>Complexity</th>
                <th>Development Estimate (days)</th>
                <th>QA Estimate (days)</th>
            </tr>
    """
    for report in repo_reports:
        summary_content += f"""
            <tr>
                ...
                <td>{report["dev_days"]}</td>
                <td>{report["qa_days"]}</td>
            </tr>
        """
    summary_content += "</table>"
    return summary_content

def create_excel_report(repo_reports, summary_report, filename):
    wb = openpyxl.Workbook()
    summary_ws = wb.active
    summary_ws.title = "Summary"

    headers = [
        "Repository Name", "Total Files", "Java Files", "PCF References",
        "Configuration Files", "Deployment Manifests", "Dependencies",
        "Java Versions", "Angular Versions", "Node Versions",
        "Complexity", "Development Estimate (days)", "QA Estimate (days)"
    ]

    summary_ws.append(headers)
    for report in repo_reports:
        summary_ws.append([
            report["repo_name"], len(report["file_details"]), report["java_files_count"],
            len(report["pcf_references"]), len(report["config_files"]),
            len(report["deployment_manifests"]), len(report["dependencies"]),
            ', '.join(set(report["java_versions"])),
            ', '.join(set(report["angular_versions"])),
            ', '.join(set(report["node_versions"])),
            report["complexity"], report["dev_days"], report["qa_days"]
        ])
    ...
    for report in repo_reports:
        sheet_title = sanitize_sheet_title(report["repo_name"])
        ws = wb.create_sheet(title=sheet_title)
        
        # File Details
        ws.append(["Name", "Path", "Size (bytes)", "Lines of Code"])
        for file_detail in report["file_details"]:
            ws.append([file_detail["name"], file_detail["path"], file_detail["size"], file_detail["lines_of_code"]])
        
        # Add additional sections for each repository
        sections = [
            ("Dependencies", report["dependencies"], ["Group ID", "Artifact ID", "Version"]),
            ("Configuration Files", report["config_files"], ["Name", "Path", "Size (bytes)", "Lines of Code"]),
            ("Deployment Manifests", report["deployment_manifests"], ["Name", "Path", "Size (bytes)", "Lines of Code"]),
            ("PCF References", report["pcf_references"], ["Name", "Path", "Size (bytes)", "Lines of Code", "Line Number", "Line Content"]),
            ("Java Versions", report["java_versions"], ["Version"]),
            ("Angular Versions", report["angular_versions"], ["Version"]),
            ("Node Versions", report["node_versions"], ["Version"]),
        ]
        
        for section_title, data, headers in sections:
            ws.append([""])
...
def main():
    ...
    try:
        ...
        create_excel_report(repo_reports, summary_report, excel_filename)
